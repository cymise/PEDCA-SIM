...
class STA:
    ...
    def channel_access(self, current_time, channel: Channel):
        ...
        for ac in ['VO', 'VI', 'BE', 'BK']:
            ...
            if self.queues[ac]:
                ...
                if self.slot_timers[ac] <= 0:
                    ...
                    if ac == 'VO' and self.PEDCA_enabled and self.backoff_counter_PEDCA is not None:
        ...
        if counter_internal_transmit > 1: #internal trasnmit 수가 1 초과이면 internal collision. internal collision이 발생하면, contention windows를 2배 늘림, QSRC 증가.
        ...
        if counter_internal_transmit == 1:
            if 'PEDCA' not in complete_EDCAF: #PEDCA가 아닌 경우
                ...
                #RTS 프레임을 설정함
                rtsframe = Template_RTS
                rtsframe['tx'] = self.id
                if self.is_ap == 0:
                    rtsframe['rx'] = 0 #수신자는 AP임
                    rtsframe['duration'] = TXOP_limit[ac] - RTS_Time - aSIFSTime - TXOP_margin  #RTS 듀레이션 설정, STA는 margin 사용 안함
                else: #AP의 RTS 프레임 전송
                    rtsframe['rx'] = txframe['rx']
                    rtsframe['duration'] = TXOP_limit[ac] - RTS_Time - aSIFSTime  #RTS 듀레이션 설정
                #데이터 프레임을 설정함
                txframe['length'] = TXOP_limit[ac] - RTS_Time - CTS_Time - response_length - TXOP_margin - 3 * aSIFSTime #데이터 프레임의 실제 전송 길이 계산 
...
####################로그출력################################
def export_sta_stats(stations, filename="sta_framelog_numsta{0}_numvo_{1}_numpedca{2}_diffAIFS{3}_dssync{4}.csv".format(num_STA, num_VO, num_PEDCA, PEDCA_AIFS, PEDCA_SYNC)):
    # CSV 필드 이름 설정
